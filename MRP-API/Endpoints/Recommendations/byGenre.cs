namespace MRP.Endpoints.Recommendations
{
    public class byGenre
    {
        ...
        public static async Task HandleByGenre(HttpListenerRequest req, HttpListenerResponse resp)
        {
            ...
            string? genre = req.QueryString["genre"];
            if (string.IsNullOrEmpty(genre))
            {
                await ClientError.Handle400(req, resp, "Missing genre in query parameters.");
                return;
            }
            ...
            try
            {
                ...
                await using var cmd = new NpgsqlCommand("SELECT m.*, AVG(r.stars) as average_rating FROM media m INNER JOIN ratings r ON m.id = r.media_id WHERE m.genre::text ILIKE @genre GROUP BY m.id ORDER BY AVG(r.stars) DESC", conn);
